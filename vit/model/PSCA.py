...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x_psca = x_without_cls.view(B,H,W,C).permute(0,3,1,2)
        x_psca = self.ca(x_psca)
        x_psca = x_psca.view(B,C,-1).permute(0,2,1)
        x_cat = torch.cat((cls_token, x_psca), dim=1)
        # 自注意力
...
class VisionTransformerPSCA(nn.Module):
    """完整的ViT模型"""
