...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x_sa = x_without_cls.view(B,H,W,C).permute(0,3,1,2)
        x_sa = self.sa(x_sa)
        x_sa = x_sa.view(B,C,-1).permute(0,2,1)
        x_cat = torch.cat((cls_token, x_sa), dim=1)
        # 自注意力
...
class VisionTransformerSA(nn.Module):
    """完整的ViT模型"""
