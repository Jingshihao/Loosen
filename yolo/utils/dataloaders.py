...
class LoadImagesAndLabels(Dataset):
    ...
    def load_mosaic(self, index):
        ...
        for i, index in enumerate(indices):
            ...
            # place img in img4
            if i == 0:  # top left
                img4 = np.full((s * 2, s * 2, img.shape[2]), 114, dtype=np.uint8)  # SAC image with 4 tiles
    ...
    def load_mosaic9(self, index):
        ...
        for i, index in enumerate(indices):
            ...
            # place img in img9
            if i == 0:  # center
                img9 = np.full((s * 3, s * 3, img.shape[2]), 114, dtype=np.uint8)  # SAC image with 4 tiles
                h0, w0 = h, w
                c = s, s, s + w, s + h  # xmin, ymin, xmax, ymax (SAC) coordinates
            elif i == 1:  # top
                c = s, s - h, s + w, s
            elif i == 2:  # top right
                c = s + wp, s - h, s + wp + w, s
