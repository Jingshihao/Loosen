...
class LA(nn.Module):
    ...
    def _process_features(self, x_means, x_maxs):
        ...
        B = x_means.shape[0]
        x_means = x_means.reshape(B, self.num_heads, 4, 4)
        x_means = self.f2(self.relu(self.f1(x_means)))

        x_maxs = x_maxs.reshape(B, self.num_heads, 4, 4)
    ...
    def _fuse_features(self, x_means, x_maxs):
        ...
        x_fusion = F.interpolate(x_fusion, (8, 8), mode='bilinear', align_corners=False)
